...
if __name__ == '__main__':
    ...
    cut_expr = '(quadlep_type>0) & (total_charge==0) & (nTaus_OR==0) & (nJets_OR_DL1r_77==0)  & (lep_Pt_0*1e-3>10) & (lep_Pt_1*1e-3>10) & (lep_Pt_2*1e-3>10) & (lep_Pt_3*1e-3>10) & (lep_isolationFCLoose_0) & (lep_isolationFCLoose_1) & (lep_isolationFCLoose_2) & (lep_isolationFCLoose_3) & (((lep_ID_0==-lep_ID_1)&(abs(Mll01-91.2e3)<10e3)&(Mll01>12e3)&(lep_ID_2==-lep_ID_3)&(abs(Mll23-91.2e3)>10e3)&(Mll23>12e3))|((lep_ID_0==-lep_ID_2)&(abs(Mll02-91.2e3)<10e3)&(Mll02>12e3)&(lep_ID_1==-lep_ID_3)&(abs(Mll13-91.2e3)>10e3)&(Mll13>12e3))|((lep_ID_0==-lep_ID_3)&(abs(Mll03-91.2e3)<10e3)&(Mll03>12e3)&(lep_ID_1==-lep_ID_2)&(abs(Mll12-91.2e3)>10e3)&(Mll12>12e3))|((lep_ID_1==-lep_ID_2)&(abs(Mll12-91.2e3)<10e3)&(Mll12>12e3)&(lep_ID_0==-lep_ID_3)&(abs(Mll03-91.2e3)>10e3)&(Mll03>12e3))|((lep_ID_1==-lep_ID_3)&(abs(Mll13-91.2e3)<10e3)&(Mll13>12e3)&(lep_ID_0==-lep_ID_2)&(abs(Mll02-91.2e3)>10e3)&(Mll02>12e3))|((lep_ID_2==-lep_ID_3)&(abs(Mll23-91.2e3)<10e3)&(Mll23>12e3)&(lep_ID_0==-lep_ID_1)&(abs(Mll01-91.2e3)>10e3)&(Mll01>12e3)))'
    
    import time
    
    s = time.time()
    all_root_files = ['/data/at3/scratch3/multilepton/VLL_production/nominal/mc16a/364140.root']
    for i, file in enumerate(all_root_files):
        ...
        for all_data in uproot.iterate(file+':nominal', library='np', allow_missing=True,step_size='1 MB'):
        ...
        l2 = time.time()
        print("Start iterating, time = ", l2-l)
        #Instead of opening the whole file, find a way to chunk it and update the ntuples? 
        for all_data in uproot.iterate(file+':nominal', library='np', allow_missing=True,step_size='100 MB'):
            
            l3 = time.time()
            print("loaded data, ",len(all_data)," time, " , l3-l2)
            ...
            df = all_data.eval(cut_expr)
            l4 = time.time()
            print("cut: ", len(df), " , time : ", l4-l3)
