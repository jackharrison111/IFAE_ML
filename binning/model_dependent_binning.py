...
from utils.data_retriever import get_data_from_feather
from binning.algs.model_dependent import run_modeldep_alg
...
def run_single_histo_binning_alg(scores, weights, num_bins=5000, bkg_scale_factors=[4]):
    
    #Start with a fine binning
    ad_binning = np.linspace(bin_min, bin_max, num_bins)
    
    #Make cumulative distributions 
    yields_sqd, bin_edges_sqd = np.histogram(scores, bins=ad_binning, weights=weights**2)
    yields, bin_edges = np.histogram(scores, bins=ad_binning, weights=weights)
    ...
    for bkg_scale_factor in bkg_scale_factors:
        ...
        #Get the point where the background is 
        ...
        #Get the points that the fractional error is minimum
        ...
        #Now find where the yield increases each time by 4 bins on the previous bin
        ...
        while(min_bkg_events < even_cumsum[0] and prev_bin_pos != 0):
                    ...
                    if len(first_bkg_point) == len(even_cumsum):  #If the yield is never bigger than the bkg events
                        print("Setting to point 0")
                        first_bkg_point = 0 #Take the first point
                    elif len(first_bkg_point) == 0:  #If the yield is never less than the min bkg events
                        #Not sure what to do...
                        first_bkg_point = first_bkg_point[-1]
                    else:
                        first_bkg_point = first_bkg_point[0] - 1
                    

                
                    first_ad_score = fine_ad_bins[first_bkg_point]
                    ...
                    min_bkg_events = even_cumsum[first_bkg_point] - even_cumsum[prev_bin_pos]
...
if __name__ == '__main__': 
    ...
    flavour_split_regions = {
       ...
       #'1Z_0b_2SFOS' : ['1Z_0b_2SFOS_EgtM', '1Z_0b_2SFOS_MgtE'],
    ...
    for region, flavour_regions in flavour_split_regions.items():
        ...
        if 'Q2' in region:
            infile = os.path.join(base_dir,region+'.ftr')
    
        if read_from_feather:
            
            dataset = get_data_from_feather(infile, r_config, region, apply_conv_veto=apply_conv_veto)
            ...
            region_scores = np.clip(region_scores, a_min=0, a_max=1)
        ...
        fine_ad_bins = np.linspace(bin_min, bin_max, 100000)
        fs = 16
        
        #Now split into flavour regions
        '''
        #########################
        The idea is to split the trained region into it's sub regions,
        and then run the algorthim over each sub_split region together, 
        ...
        #Run over split regions
        if flavour_regions[0] != region:
        
            #Next we want to make all of the sub regions histograms that we need.
            split_regions_data = {}
            ...
            #Merged data now contains flavour information 
            
            #Get the required events for the flavour split region
            for sub_region in flavour_regions:
                ...
                if '0Z_0b_1SFOS' in sub_region or '0Z_1b_1SFOS' in sub_region:
                    ...
                    if 'EgtM' in sub_region:
                        ...
                        sub_r_data['Bkg'] = merged_data.loc[merged_data['quadlep_type']==4]
                        #sub_r_data['Sig'] = merged_sig_data.loc[merged_sig_data['quadlep_type']==4]

                    elif 'MgtE' in sub_region:

                        sub_r_data['Bkg'] = merged_data.loc[merged_data['quadlep_type']==2]
                        #sub_r_data['Sig'] = merged_sig_data.loc[merged_sig_data['quadlep_type']==2]

                elif '0Z_0b_2SFOS' in sub_region or '0Z_1b_2SFOS' in sub_region:

                    if 'EgtM' in sub_region:
                        sub_r_data['Bkg'] = merged_data.loc[merged_data['quadlep_type']==5]
                        #sub_r_data['Sig'] = merged_sig_data.loc[merged_sig_data['quadlep_type']==5]

                    elif 'EeqM' in sub_region:
                        sub_r_data['Bkg'] = merged_data.loc[merged_data['quadlep_type']==3]
                        #sub_r_data['Sig'] = merged_sig_data.loc[merged_sig_data['quadlep_type']==3]

                    elif 'MgtE' in sub_region:
                        sub_r_data['Bkg'] = merged_data.loc[merged_data['quadlep_type']==1]
                        #sub_r_data['Sig'] = merged_sig_data.loc[merged_sig_data['quadlep_type']==1]
                    ...
                    if 'EgtM' in sub_region:
                        #quadlep_type = 5
                        sub_r_data['Bkg'] = merged_data.loc[(merged_data['quadlep_type'] == 5)|((merged_data['Ztype'] == 'M')&(merged_data['quadlep_type'] == 3))]
                        #sub_r_data['Sig'] = merged_sig_data.loc[(merged_sig_data['quadlep_type'] == 5)|((merged_sig_data['Ztype'] == 'M')&(merged_sig_data['quadlep_type'] == 3))]
                ...
                #Now save all the sub_regions into one place so we can loop over them together
                split_regions_data[sub_region]  = sub_r_data 
               
                str_alg_bins = ['%s' % float('%.5g' % a) for a in binnings]
                ...
                print(yield_str)
                with open(os.path.join(out_dir, f"{sub_region}_OutputBins.txt"), 'w') as f:
                ...
                plt.yscale('log')
                #plt.title(region)
                plt.legend(title=sub_region)
                plt.ylim(bottom=0)
                plt.savefig(os.path.join(out_dir, 'AD_scores_NewBins_LOG.png'), dpi=300)
                plt.savefig(os.path.join(out_dir, 'AD_scores_NewBins_LOG.pdf'), dpi=300)
                plt.close()

                #Make some plots and show them?
                 
                
                ...
            #Now pick the sub_region with the largest first bin
            largest_bin = None
            best_region = None
            
            for i, binnings in split_regions_data.items():
                bins = binnings['Binning']
                bin_width = bins[-1] - bins[-2]
                print("Got bin width: ", bin_width)
                if largest_bin is not None:
                    if bin_width > largest_bin:
                        largest_bin = bin_width
                        best_region = i
                else:
                    largest_bin = bin_width
                    best_region = i

            best_bins = split_regions_data[best_region]['Binning']

            for i, sub_r_data in split_regions_data.items():
                #Plot them all with the same binning
                plt.figure()
                ...
                plt.legend(title=i)
                plt.ylim(bottom=0)
                plt.savefig(os.path.join(out_dir, f'AD_scores_FixedBins_LOG_{i}.png'), dpi=300)
                plt.savefig(os.path.join(out_dir, f'AD_scores_FixedBins_LOG_{i}.pdf'), dpi=300)
                plt.close()
                
            #Make some plots with the best region?
            
            

            
        
            #Next we run the algorthim over the three histograms at the same time.
            #Now make lists of the points where the algorithm is accepted for each one,
            #and take the left-most point as the first bin. 
            
            #Then increase the yield by a factor of 4 from there, but with which yield? and to make one binning? 
            
            #This ensures that the binning is 'atleast' for each region.
        
        else:

            #Run the binning alg normally
            binnings, bin_yields = run_modeldep_alg(region_scores, region_weights)
            ...
            yield_str = f"Bin yields: {y_str}\n"
            print(yield_str)
            with open(os.path.join(out_dir, f"{region}_OutputBins.txt"), 'w') as f:
                f.writelines([bin_str, yield_str])

            plt.figure()
            counts, edges, _ = plt.hist(region_scores, 
                                        bins=binnings, 
                                        weights=region_weights, alpha=0.8)
            plt.ylabel('Counts', fontsize=fs)
            plt.xlabel('Anomaly score', fontsize=fs)
            plt.yscale('log')
            #plt.title(region)
